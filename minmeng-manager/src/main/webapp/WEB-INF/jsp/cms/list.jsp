...
<body>
	...
	<div id="tb" style="padding: 2px 5px;">
		<a href="javascript:reload()" class="easyui-linkbutton"
			...
			href="javascript:edit('添加内容');" class="easyui-linkbutton"
			data-options="iconCls:'icon-add',plain:true">添加</a> <a
			href="javascript:modi();" class="easyui-linkbutton"
			data-options="iconCls:'icon-edit',plain:true">修改</a> <a
			href="javascript:remove();" class="easyui-linkbutton"
			data-options="iconCls:'icon-remove',plain:true">删除</a> <c:if test="${cid=='201901281133158053'}"><a
			href="javascript:relase();" class="easyui-linkbutton"
			data-options="iconCls:'icon-redo',plain:true">同步地州</a> </c:if><input
			id="searchbox" style="width: 150px"></input> <c:if test="${cid=='201907121639414046'}"><a
			href="javascript:modi2();" class="easyui-linkbutton"
			data-options="iconCls:'icon-edit',plain:true">选择推送栏目</a></c:if>
	...
	<script type="text/javascript">
		...
		}function edit2(title, id) {
			id = id ? id : "";
			var url = '${path}/edit?id=' + id + '&cid=${cid}';
			dialog = $.dialog({
				title : title,
				content : 'url:' + encodeURI(url),
				lock : true,
		...
		function fabu(ss) {
			...
			console.log(ss);
			$.messager.confirm('提示', '你确定要报送到民盟省委吗?', function(r) {
				if (r) {
					// 提交URL
					var url = '${path }/release';
		...
		$(function() {
			...
			var o  = {
				...
				columns : [ [
					...
					},
					{
						...
						sortable : true,
						formatter : function(value, row) {
							return row.files ? row.files.length
									: 0;
						}
					},
					{
			...
			if(cid!='201901281133158053'){
				o.columns[0] = o.columns[0].filter(function (element){
					return !(element.field == "release");
				});
			}
			console.log(o)
			$('#dg')
