...
//		path = path.substring(path.indexOf('/'),path.length);
	...
	var _uploadDoms ={
	 ...
	 $.fn.fixupload = function(options) {
		...
		// 属性
		var defaults = {
			resizeImg:'false',		// 压缩图片
			resizeImgWidth:0,		// 压缩图片宽度
			resizeImgHeight:0,		// 压缩图片宽度
			resizeImgQuality:0.8,			// 压缩图片精度
			...
			init:function(){
				if(config.autoCreate){					
					...
					swfupload = new SWFUpload({
						...
						url_prefix 				: config.urlPrefix?config.urlPrefix:'',
...
//					_iframe.setAttribute('width',DOM.width());
			...
			createUpload : function(index) {
				...
				if(config.urls){
					if(!$.isArray(config.urls)){
						config.loadFilename($dom,$input,config.urls);
					} else if(config.urls[index]){
						$input.val(config.urls[index]);
						if($dom.find('.uploadImage')){
							// 默认图片
							$dom.find('.uploadImage img').attr('src',(config.urlPrefix?config.urlPrefix:'')+config.urls[index]);
							$dom.find('.click').text(config.buttonSuccessText);
						}else{
							config.loadFilename($dom,$input,config.urls[index]);
						}
					}
				}
				$dom.append($input);
			...
			loadFilename:function(_dom,_input,url){
				var filename = url.substring(url.lastIndexOf('/')+1,url.length);
				_input.val(url);
				_dom.find('.progress').animate({width : "100%"}, "fast");
				_dom.find(".state").html('<span class="filename">'+filename+ ' &nbsp;</span>');
				config.setIcon(filename,function(url){
					_dom.find('.icon').find('img').attr('src',(config.urlPrefix?config.urlPrefix:'')+url);	
					_dom.find('.icon').show();
				});
			},
			...
			createFileProgress:function(file,_uploadContainer){
				...
				if(_icon.size()>0){
					...
					config.setIcon(file.name,function(url){
						_icon.find('img').attr('src',(config.urlPrefix?config.urlPrefix:'')+url);	
						_icon.show();
			...
			successUpload:function(file,serverData){
				var data = serverData;
				if(serverData && typeof(serverData) == 'string' && serverData.replace(/[\r\n\s]/g, "")!=''){
			...
			successImgUpload:function(file,data){
				...
				imgNode.attr('src',(config.urlPrefix?config.urlPrefix:'')+data.info.url);
				if(config.success){
					config.success(config.getUrls());
				}
				config.resizeState(DOM.find('#'+file.id).find(".state"));
			...
			loadImgUpload:function(id,url,imgNode){
				...
				progress.hide();
				state.hide();
				imgNode.attr('src',(config.urlPrefix?config.urlPrefix:'')+url);
				if(config.success){
					config.success(config.getUrls());
				}
			},
