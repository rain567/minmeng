...
<div class="view ${view.style }">
	...
	<div class="view-list">
		...
		<c:forEach items="${data}" var="entry">
				<div class="list-tab"><span>${entry.key.name}</span></div>
		</c:forEach>
		</div>
		<div class="list-contents">
		<c:forEach items="${data}" var="entry">
			<div class="list-item">
				<div class="list-item-icon" style="background-image:url(${ATTACH_PREFIX}/${entry.key.icon});">
				<a href="${path}/c/${entry.key.uri}"></a>
				</div>
				...
				<c:forEach items="${entry.value }" var="item" varStatus="status">
				   ...
				   <li>
					   	<a href="${href }">
							<span class="item-img" style="<c:if test="${fn:length(item.images) > 0 }">background-image:url(${ATTACH_PREFIX}/${item.images[0].url });</c:if>background-repeat: no-repeat; background-position:center;background-size:cover;"></span>
